...
//новая карточка кандидата
	btnReady.addEventListener('click', function() {
		...
		newCardsItem.classList.add('main-cards-item-active');
...
//кнопка вмешаться в выборы
	btnСrime.addEventListener('click', function() {
		//добавление +25% нашему кандидату
	/*let progressBar = mainBlock.querySelectorAll('.progress-bar'),
			progressBarNumber = mainBlock.querySelectorAll('.result-count');
		for (var i = 0; i < progressBar.length; i++) {
			if (i == 2) {
				if (parseInt(progressBar[i].style.height) >= 0 && parseInt(progressBar[i].style.height) <= 75) {
					progressBar[i].style.height = parseInt(progressBar[i].style.height) + 25 + "%";
					progressBarNumber[i].textContent = progressBar[i].style.height;
				}
			}
		};*/
